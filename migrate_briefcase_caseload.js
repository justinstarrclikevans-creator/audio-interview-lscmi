/**
 * migrate_briefcase_caseload.js
 * 
 * Port caseload and completed data from Briefcase app (Supabase) to current app (SQLite).
 * - Queries Supabase participants table
 * - Normalizes and merges duplicates (semaj hart, Hakeem Alston, Andres/Andresd Bonham)
 * - Excludes system records (_GLOBAL_SETTINGS_)
 * - Inserts or updates SQLite users, participant_profiles, briefcase_items, gate_criteria, resumes
 * - Maps all coreStability, employmentReadiness, healthWellness, financial, careerPlanning, and wo_progress
 */

const https = require('https');
const path = require('path');
const bcrypt = require('bcryptjs');
const { db, BRIEFCASE_DOMAINS, DEFAULT_GATE_CRITERIA, initParticipantBriefcase } = require('./db.js');
...
async function runMigration() {
    console.log('🚀 Starting Briefcase App Caseload Migration...');
    const rawList = await fetchSupabaseParticipants();
    console.log(`📥 Fetched ${rawList.length} total rows from Supabase.`);
    ...
    console.log(`📋 Found ${validRows.length} valid participant records.`);

    // Deduplicate & deep merge
    ...
    for (const row of validRows) {
        ...
        if (!mergedMap.has(key)) {
            ...
            // Merge state data favoring non-empty/true values
            ...
            // If either had true for a flag, keep true
    ...
    console.log(`✨ Total unique individuals to migrate: ${mergedMap.size}`);

    const defaultPasswordHash = bcrypt.hashSync('Turn902026!', 10);
    ...
    const migrateTx = db.transaction(() => {
        for (const [key, p] of mergedMap.entries()) {
            ...
            let user = db.prepare('SELECT * FROM users WHERE LOWER(name) = ? OR LOWER(email) = ?').get(p.fullName.toLowerCase(), email);
            ...
            if (user) {
                ...
                `).run(p.fullName, email, phone, defaultPasswordHash, p.location);
            ...
            // Initialize briefcase items & gate criteria
            ...
            if (cs.professionalEmail === true) updateBriefcase.run('green', `Email: ${cs.emailAddress || email}`, userId, 'prof_email');
            ...
            if (cs.professionalEmail === true) updateBriefcase.run('green', 'Professional email verified', userId, 'prof_email_created');
            ...
            // 6. Credentials / Trades Training progress
            ...
            // Save Master Resume if available
            if (er.resumeData && (er.resumeData.skills || er.resumeData.summary || er.resumeData.experience || (er.resumeData.jobs && er.resumeData.jobs.length > 0))) {
                ...
                // Mark Gate 3 resume criterion green
                db.prepare(`
                    UPDATE gate_criteria SET status = 'green', pm_notes = 'Master resume ported from Briefcase app'
            ...
            if (cs.stateId === true || cs.ssnCard === true || cs.birthCertificate === true) {
                updateGate.run('green', 'State ID / Identification on file', userId, 'w1_w9_id');
                db.prepare(`UPDATE participant_profiles SET w9_status = 'verified' WHERE user_id = ?`).run(userId);
            }

            // W1 90-Day Goal & Email
            if (cp.careerGoal || cp.sixMonthGoal || cs.professionalEmail === true) {
                updateGate.run('green', cp.careerGoal || '90-day goal & email established', userId, 'w1_goal_email');
            }

            // W2 Driver's license / Child support plan
            ...
            // Calculate current gate based on progress
            let currentGate = 1;
    ...
    console.log('\n📊 Caseload Breakdown by Location in SQLite:');
    summary.forEach(s => console.log(`   - ${s.location}: ${s.count} participants`));

    const totalParticipants = db.prepare("SELECT COUNT(*) as count FROM users WHERE role = 'participant'").get().count;
    const resumesCount = db.prepare("SELECT COUNT(*) as count FROM resumes").get().count;
    const completedBriefcaseItems = db.prepare("SELECT COUNT(*) as count FROM briefcase_items WHERE status = 'green'").get().count;

    console.log(`\n🎉 Summary Stats:`);
    console.log(`   - Total Caseload Participants: ${totalParticipants}`);
    console.log(`   - Ported Resumes: ${resumesCount}`);
    console.log(`   - Verified Briefcase Items: ${completedBriefcaseItems}`);
}
